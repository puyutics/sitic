...
use app\models\parkingcontrol\AsistnowSearch;
...
class AsistnowController extends Controller
{
    ...
    public function actionIndexuser($oc_user_id, $oc_zona = NULL, $oc_zona_parqueo = NULL)
    {
        $oc_user_id = base64_decode($oc_user_id);
        $oc_zona = base64_decode($oc_zona);
        $oc_zona_parqueo = base64_decode($oc_zona_parqueo);
        $searchModel = new AsistnowSearch();
        $dataProvider = $searchModel->search(Yii::$app->request->queryParams);
        if ($oc_zona != NULL AND $oc_zona_parqueo != NULL) {
            $dataProvider->query->orWhere(['ASIS_ID' => $oc_user_id, 'ASIS_ZONA' => $oc_zona]);
            $dataProvider->query->orWhere(['ASIS_ID' => $oc_user_id, 'ASIS_ZONA' => $oc_zona_parqueo]);
        } else {
    ...
    public function actionIndexpuerta($oc_zona, $oc_zona_parqueo)
    {
        $oc_zona = base64_decode($oc_zona);
        $oc_zona_parqueo = base64_decode($oc_zona_parqueo);
        $searchModel = new AsistnowSearch();
        $dataProvider = $searchModel->search(Yii::$app->request->queryParams);
        $dataProvider->query->orWhere(['ASIS_ZONA' => $oc_zona]);
        $dataProvider->query->orWhere(['ASIS_ZONA' => $oc_zona_parqueo]);
        $dataProvider->sort->defaultOrder = ['ASIS_ING' => SORT_DESC,];
        $dataProvider->pagination = ['pageSize' => 50];
