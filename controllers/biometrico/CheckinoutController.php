...
namespace app\controllers\biometrico;
...
class CheckinoutController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        $dataProvider->sort->defaultOrder = ['CHECKTIME' => SORT_DESC,];
    ...
    public function actionIndexsensor($sensor_id)
    {
        $sensor_id = base64_decode($sensor_id);
        $searchModel = new CheckInOutSearch();
        $dataProvider = $searchModel->search(Yii::$app->request->queryParams);
        $dataProvider->query->andWhere(['SENSORID' => $sensor_id]);
        $dataProvider->sort->defaultOrder = ['CHECKTIME' => SORT_DESC,];
        $dataProvider->pagination = ['pageSize' => 100];

        return $this->render('indexsensor', [
            'searchModel' => $searchModel,
            'dataProvider' => $dataProvider,
        ]);
    }
